...
-   Percentage of main water point technology (i.e. Hand Pump, Mechanised Pump)
...
![Example of Handpump](data2/bore-holes-ntic.jpg)
...
![Example of Mechanised Pump](data2/mechpump.jfif){width="1280"}
...
![Example of Tapstand](data2/IMG_0187.jpg)

![Example of Rope and Bucket](data2/LagosIsland-032021-Nigeria-NengiNelson-SL-06.jpg)
...
To have a quick look at the distribution of functional and non-functional water points at township level, a choropleth map will be prepared.

The code chunks below are used to prepare the choropleth map by using the *qtm()* function of **tmap** package.

```{r}
wpt.functional.map <- tm_shape(nga_wp) + 
...
tmap_arrange(wpt.functional.map, wpt.pct.map,
             asp=NA, ncol=2)
```

Notice that the choropleth maps above clearly show that townships with relatively larger number of functional waterpoints are also showing relatively higher percentage of functional waterpoints. And the maps also seem to suggest higher percentages of functional waterpoints are mainly in the Northern townships.

Now let us plot the choropleth maps showing the distribution of number of non-functional waterpoints and the percentage of non-functional waterpoints using the code chunk below.

```{r}
wpt.nfunctional.map <- tm_shape(nga_wp) + 
...
tmap_arrange(wpt.nfunctional.map, wpt.npct.map,
             asp=NA, ncol=2)
```

Notice that the choropleth maps above show that townships with relatively larger number of non-functional waterpoints may not show a relatively higher percentage of non-functional waterpoints. And the maps also seem to suggest higher percentages of non-functional waterpoints are mainly in the Southern townships.
...
From the output above, the row number has been replaced with township name.
...
From the histograms above, we can indeed see that there is no difference between the raw values and min-max standardised values of pct_handpump, which supports the earlier notion that there is no need to plot the remaining variables that are in percentages.
...
With reference to the gap statistic graph above, the recommended number of cluster to retain is 1. However, it is not logical to retain only one cluster. By examining the gap statistic graph, the 4-cluster gives the largest gap statistic and should be the next best cluster to pick.
...
heatmaply(normalize(nga_wp_pct_mat),
          ...
          xlab = "Water Technology Indicators",
...
With closed examination of the dendrogram above, we will retain the 4 clusters. Using the code chunk below, we will derive a 4-cluster model using cutree() of Base R.
...
The choropleth map above reveals the clusters are very fragmented. This is one of the major limitations when non-spatial clustering algorithm such as hierarchical cluster analysis method is used.
...
Note that the dimension is 772 and not 773. This is because the minimum spanning tree consists of n-1 edges (links) in order to traverse all the nodes.
...
The boxplots reveal that Cluster 4 displayed the highest mean for both wpt_functional and wpt_non-functional. This is followed by Cluster 2, 3 then 1.
...
The parallel coordinate plot above reveals that households in Cluster 4 townships tend to have the highest percentage of (1) functional waterpoints, (2) handpumps and (3) waterpoints located in rural areas.

On the other hand, households in Cluster 1 tends to have the highest percentage of (a) non-functional waterpoints, (b) mechanised pumps, and (c) waterpoints located in urban areas.
...
Last but not least, we can also compute the summary statistics such as mean, median, sd, etc to complement the visual interpretation. Using the code chunk below, `group_by()` and `summarise()` of dplyr are used to derive mean values of the clustering variables.
...
nga_wp_ngeo_cluster %>% 
  ...
  summarise(mean_wpt_functional = mean(wpt.functional),
            ...
            mean_pct_usage1000more = mean(pct_usage1000more),
            mean_pct_usage1000less = mean(pct_usage1000less),
            mean_pct_rural = mean(pct_rural))
```

The table above supports our earlier observations that (1) Cluster 4 has both the highest number of functional waterpoints as well the highest percentage of functional waterpoints, and these waterpoints are mainly handpumps, and are located in the rural areas; (2) on the other hand, while Cluster 1 has the lowest number of non-functional waterpoints, they have the highest percentage of non-functional waterpoints, and these waterpoints are mainly mechanised pumps, and are located in the urban areas.

# 9. Acknowledgements

Sincere appreciation to Prof. Kam Tin Seong for all the lecture notes,hands-on exercises and in-class assignments. Heavy references were made from data wrangling, mapping and clustering!

## 9.1 Resources used

[ONG ZHI RONG JORDAN](https://jordan-isss624-geospatial.netlify.app/posts/geo/geospatial_exercise/), Geospatial Analytics for Social Good - Understanding Nigeria Water functional and non-functional water point rate

***Photo Credits***:

-   Nengi Nelson, "Waterpreneurs" cash in on government failure to deliver clean water to islanders
