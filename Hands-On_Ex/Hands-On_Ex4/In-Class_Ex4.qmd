...
**Geographically weighted regression (GWR)** is a spatial statistical technique that takes non-stationary variables into consideration (e.g. climate; demographic factors; physical environment characteristics) and models the local relationships between these independent variables and an outcome of interest (also known as dependent variable). In this hands-on exercise, we will learn how to build [hedonic pricing](https://www.investopedia.com/terms/h/hedonicpricing.asp) models using GWR methods. The dependent variable is the resale prices of condominium in 2015. The independent variables are divided into either structural and locational.
...
-   R package for building OLS regression models and performing diagnostics tests to test assumptions
...
## 7.2 Multiple histogram plot distribution of variables
...
The code chunk below is used to create 12 histograms. Then, `ggarrange()` is used to organised these histogram into a 3 columns by 4 rows of small multiple plot.
...
Next, the code chunks below is used to create an interactive point symbol map. *As some of the polygons do not have a neighbour (as the coordinates are not error-free), we will overwrite this using " tmap_options(check.and.fix = TRUE) immediately after tm_shape().*
...
The code chunk below is used to plot a scatterplot matrix of the relationship between the independent variables in *condo_resale* data.frame. *Note that the **sf dataframe is not used** here.*
...
*From the output above, it is statistically significant at alpha = 0.05, and since adjusted R-square is 0.647, which means that the independent variables are able to explain for 64.7% of the variation in selling price.*
...
*Similarly, from the output above, it is statistically significant at alpha = 0.05, and since adjusted R-square is 0.647, which means that the independent variables are able to explain for 64.7% of the variation in selling price. Using AGE as an example, one additional unit of AGE will reduce the selling price by \$24,687.739.*
...
First, we will export the residual of the hedonic pricing model and save it as a tibble data frame.
...
***Note: Remember to switch back to "plot" mode before continue.***
...
The result shows that the recommended bandwidth is 971.3405 metres. *(SI for GWR is in metres)*
